...
bot = telebot.TeleBot('6583151106:AAEckYcrco3qa9gCQ5UxXKQuF5QL4-Ys8YY')
    ...
    if user_id in allowed_user_ids:
        ...
        if user_id not in admin_id:
            ...
            if user_id in bgmi_cooldown and (datetime.datetime.now() - bgmi_cooldown[user_id]).seconds < COOLDOWN_TIME:
                response = "You Are On Cooldown. Please Wait 150 sec Before Running The /attack Command Again buy premium instant attack with zero sec dm @spyther."
                bot.reply_to(message, response, reply_markup=get_inline_keyboard())
                return
